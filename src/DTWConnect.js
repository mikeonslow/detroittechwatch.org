...
class DTWConnect extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {value: '', validation: "Initial", subscribeMessage: '', remoteData: "NotAsked"};

    this.handleChange = this.handleChange.bind(this);
    this.handleSubmit = this.handleSubmit.bind(this);
    this.handleSubscriberFormSuccess = this.handleSubscriberFormSuccess.bind(this);
  ...
  handleChange(e) {
    ...
    const subscriberMessage = validation === "Valid" ? validationSuccessMessage : validationErrorMessage;
    this.setState({value: e.target.value, validation: validation, subscribeMessage: subscriberMessage});
  }
  ...
  handleSubscriberFormSuccess(data) {
    ...
    this.setState({remoteData: remoteData, validation: validation, subscribeMessage: responseData.message});

    setTimeout(this.resetForm, 5000);
  }

  resetForm() {
    this.setState({value: '', remoteData: "NotAsked", validation: "Initial", subscribeMessage: ""});
  }
  ...
  render() {
    ...
    switch (validation) {
      case "Invalid":
        validationFeedback =
            <div className="text-danger text-left"><MDBIcon icon="exclamation-circle"/> {subscribeMessage}</div>;
        break;
      case "Valid":
        validationFeedback = <div className="text-success text-left"><MDBIcon icon="check"/> {subscribeMessage}</div>;
        break;
      default:
        validationFeedback = '';
    }
